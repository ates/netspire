...
do_ha1(UserName, Password, Nonce, CNonce, Request) ->
    ...
    HA = string:to_lower(util:binary_to_hex_string(H)),
    
    if
        Algorithm == <<"MD5-sess">> orelse Algorithm == <<"MD5-Sess">> ->
            Hash = crypto:md5([HA, ":", Nonce, ":", CNonce]),
            string:to_lower(util:binary_to_hex_string(Hash));
        true -> HA
    end.
...
do_digest(HA1, Nonce, CNonce, Request) ->
    ...
    HA2Hex = string:to_lower(util:binary_to_hex_string(HA2)),
    ...
    string:to_lower(util:binary_to_hex_string(RespHash)).
