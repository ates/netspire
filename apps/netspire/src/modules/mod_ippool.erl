...
add_framed_ip(Response) ->
    case radius:attribute_value("Framed-IP-Address", Response) of
        undefined ->
            ...
            case lease(Pool) of
                {ok, IP} ->
                    ?INFO_MSG("Adding Framed-IP-Address ~s~n", [?IP2STR(IP)]),
...
renew_framed_ip(Request) ->
    ...
    case renew(IP) of
        {ok, _} ->
            ?INFO_MSG("Framed-IP-Address ~s is renewed~n", [?IP2STR(IP)]);
        {error, not_found} ->
            ok;
        {error, Reason} ->
            ?WARNING_MSG("Cannot renew Framed-IP-Address ~s"
                "due to ~p~n", [?IP2STR(IP), Reason])
    end.
...
release_framed_ip(Request) ->
    ...
    case mnesia:dirty_read({ippool, IP}) of
        [Rec] ->
            ?INFO_MSG("Release Framed-IP-Address ~s~n", [?IP2STR(IP)]),
