...
init([Port, IP, Family]) ->
    ...
    case gen_udp:open(Port, SocketOpts) of
        {ok, Socket} ->
            ?INFO_MSG("Starting RADIUS service on ~s:~p~n", [?IP2STR(IP), Port]),
            ...
            {ok, #state{socket = Socket, clients = Clients}};
        {error, Reason} ->
            Msg = inet:format_error(Reason),
            ?ERROR_MSG("Unable to start RADIUS service: ~s~n", [Msg]),
            {stop, Reason}
    end.
    ...
    case lookup_client(SrcIP, State#state.clients) of
        {ok, Client} ->
            case radius:decode_packet(Bin, Client#nas_spec.secret) of
                ...
                {error, invalid} ->
                    ?WARNING_MSG("Invalid packet from NAS: ~s~n", [?IP2STR(SrcIP)]),
                    {noreply, State}
            end;
        undefined ->
            ?WARNING_MSG("Request from unknown client: ~s~n", [?IP2STR(SrcIP)]),
            {noreply, State}
    end;
