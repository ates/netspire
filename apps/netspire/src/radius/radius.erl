                    ...
                    Value -> % Verify Message-Authenticator attribute
                        ...
                        case crypto:md5_mac(Secret, Packet1) =:= Value of
                            ...
                            false ->
                                ...
                                {error, invalid_radius_ma}
                        end
                end;
            false ->
                ?ERROR_MSG("Invalid size of RADIUS packet~n", []),
                {error, invalid_radius_packet_size}
        end
    catch
        _:Reason ->
            ?ERROR_MSG("Can not decode RADIUS packet: ~p~n", [Reason]),
            {error, invalid_radius_packet}
    end.
...
%% @doc Decode Erlang terms to RADIUS binary
...
decode_value(Bin, Length, ipv6addr) ->
    <<Value:Length/binary, Rest/binary>> = Bin,
    {ip:bin_ipv6_to_address(Value), Rest};
decode_value(Bin, _Length, ipv6prefix) ->
    <<0:8, Prefix:8, IP/binary>> = Bin,
    Address = ip:bin_ipv6_to_address(IP),
    {{Prefix, Address}, <<>>};
